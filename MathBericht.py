...
def math_stundenrechnung(mitarbeiter):
    ...
    for person in mitarbeiter:
        ...
        verletzung = vertragsbedingungen(gesamt_effektiv, gesamt_pausen, pensum)
        if verletzung:
...
# Check Vertragsbedingung
def vertragsbedingungen(EffektivArbeitsStunden, AnzahlPausenStunden, Pensum):
    # Max-Werte pro Woche
    max_stunden = 48.0  # bei 100%
    max_pausen_anzahl = 5.0  # hier als Stundenanzahl (so wie du es vorgesehen hattest)

    # Pensum in Prozent -> Faktor 0..1
    pensum_faktor = Pensum / 100.0

    # Check Conditions der Variablen
    if (
        EffektivArbeitsStunden > max_stunden * pensum_faktor
        or AnzahlPausenStunden > max_pausen_anzahl
    ):
        return True
    else:
        return False
...
if __name__ == "__main__":
    # HIER deinen echten CSV-Pfad eintragen:
